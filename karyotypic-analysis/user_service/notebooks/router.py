...
async def get_notebook_by_id(
    ...
    notebook_dict = {
        ...
        "image_src": (
            base64.b64encode(notebook_data.image_src).decode("utf-8")
            if notebook_data.image_src
            else None
        ),
...
async def update_notebook_by_id(
    ...
    db_notebook_data = {"title": notebook.title, "user_id": current_user.id}
    update_notebook_stmt = (
        update(Notebook).where(Notebook.id == notebook_id).values(**db_notebook_data)
    )
    ...
    if notebook.comment:
        ...
        update_comment_stmt = (
            update(Comment)
            .where(Comment.notebook_id == notebook_id)
            .values(**db_comment_data)
        )
